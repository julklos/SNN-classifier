...
data_x=importdata('zestaw_12-train_set.txt');
%%3 wyrzucone, bo korelowało się z 1
data = data_x(:,[1,2,4,5]);
...
data_center=zeros(sample_count,var_count);
...
min_max = zeros(var_count,2);

for i=1:1:var_count
   avg(i)=mean(data(:,i));
   %min_max(i,:) = [min(data(:,i)), max(data(:,i))];
   %data_norm(:,i)=(data(:,i)-min(data(:,i)))/(max(data(:,i))-min(data(:,i)));
   %data_center(:,i)=data(:,i)-avg(i);
...
for class_no=1:1:class_count
    ...
    idx_pos=find(data_x(:,end)==class_no);
    idx_neg=find(data_x(:,end)~=class_no);
    plot(data(idx_pos,var_ranking(class_no,1)),data_x(idx_pos,var_ranking(class_no,2)),'ro');
    hold on;
    plot(data(idx_neg,var_ranking(class_no,1)),data_x(idx_neg,var_ranking(class_no,2)),'k.');
end
